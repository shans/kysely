...
import type { IsolationLevel, AccessMode, Driver } from './driver/driver.js'
...
import { isObject } from '../shared/util/object-utils.js'
...
// Symbol.asyncDispose is part of the TC39 Explicit Resource Management proposal and enables
// the `await using db = new Kysely(...)` syntax. It is natively available in Node ≥ 18.20 /
// Node ≥ 20.4. This polyfill creates the symbol on older runtimes so the `[Symbol.asyncDispose]`
// method declared on Kysely<DB> below is reachable at runtime. The assignment is guarded by ??=
// so it is a true no-op when the symbol already exists. This one runtime side-effect is the sole
// reason this file lives in types/ rather than being pure type declarations.
// @ts-ignore
