...
import { KyselyComponent, type KyselyComponentConfig, type TxAction, type StreamAction, type StreamStartPayload } from './kysely-component.js'
...
function createKyselyInstance(config: KyselyComponentConfig): any {
  const component = new KyselyComponent(config)
  ...
  executorShim = new ComponentQueryExecutor(ctx, config.dialect.createAdapter(), config.plugins ?? [])
