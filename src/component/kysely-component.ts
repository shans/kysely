...
export class KyselyComponent extends Component {
  ...
  // Transaction lifecycle: begin (acquire connection, start tx, pin to txId tag),
  // commit/rollback (end tx, release connection), savepoint/rollbackSavepoint/releaseSavepoint
  // (SQL savepoint commands on the pinned connection, tagged with txId).
  readonly transactionIn: ChannelIn<TxAction>
  // Connection pinning without a transaction: begin (acquire and pin to connId payload),
  // end (release, tagged with connId). Used by db.connection() to share one connection
  ...
  readonly driver: KyselyExecutorComponent['driver']

  constructor(config: KyselyComponentConfig) {
    super()
    const ex = this.executor = new KyselyExecutorComponent(config)
